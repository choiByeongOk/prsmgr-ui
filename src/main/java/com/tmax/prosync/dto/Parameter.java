...
public class Parameter extends DataObject {
	private static final String DTO_LOGICAL_NAME = "Parameter";
	...
	public Parameter fillKeyValue(int _index) {
	...
	public Object clone() {
		Parameter copyObj = new Parameter();	
		copyObj.clone(this);
		return copyObj;
	}
	public void clone(DataObject _parameter) {
		if(this == _parameter) return;
		Parameter __parameter = (Parameter) _parameter;
		
		this.setTopId(__parameter.getTopId());
		this.setProcessType(__parameter.getProcessType());
		this.setProcessNum(__parameter.getProcessNum());
		this.clearKeyValue();
		for (int index = 0; index < __parameter.sizeKeyValue(); index++) {
			this.addKeyValue(index, (com.tmax.prosync.dto.KeyValue)__parameter.getKeyValue(index).clone());		
		}
		this.setType(__parameter.getType());
